...
object Function {
  ...
  /**
    ...
    */
  def map2[T1, T2, R](t1y: Try[T1], t2y: Try[T2])(f: (T1, T2) => R): Try[R] = t1y flatMap (a => t2y map (b => f(a,b))) 
 // TO BE IMPLEMENTED
  ...
  /**
    ...
    * @return a value of R, wrapped in Try
    */
  def map3[T1, T2, T3, R](t1y: Try[T1], t2y: Try[T2], t3y: Try[T3])(f: (T1, T2, T3) => R): Try[R] = t1y flatMap (a => t2y flatMap (b => t3y map (c => f(a,b,c))))
 // TO BE IMPLEMENTED
  ...
  t1y flatMap (a => t2y flatMap (b => t3y flatMap (c => t4y flatMap(d => t5y flatMap (e => t6y flatMap (g => t7y map (h => f(a,b,c,d,e,g,h))))))))
 // TO BE IMPLEMENTED
 ...
 // TO BE IMPLEMENTED
  ...
  def lift2[T1, T2, R](f: (T1, T2) => R): (Try[T1], Try[T2]) => Try[R] = (t1,t2) => t1 flatMap(a => t2 map (b => f(a,b))) 
 // TO BE IMPLEMENTED
  ...
  def lift3[T1, T2, T3, R](f: (T1, T2, T3) => R): (Try[T1], Try[T2], Try[T3]) => Try[R] = (t1,t2,t3) => t1 flatMap (a => t2 flatMap (b => t3 map (c => f(a,b,c))))
 // TO BE IMPLEMENTED
 ...
 (t1,t2,t3,t4,t5,t6,t7) => t1 flatMap (a => t2 flatMap (b => t3 flatMap (c => t4 flatMap (d => t5 flatMap (e => t6 flatMap (g => t7 map (h => f(a,b,c,d,e,g,h)))))))) 
 // TO BE IMPLEMENTED
