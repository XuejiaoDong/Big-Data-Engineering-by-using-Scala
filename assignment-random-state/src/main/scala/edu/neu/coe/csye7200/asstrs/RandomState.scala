...
trait RandomState[T] {
  ...
  // 10 points
  def flatMap[U](f: T => RandomState[U]): RandomState[U] = f(this.get)// TO BE IMPLEMENTED
  ...
  // 12 points
  def toStream: LazyList[T] = LazyList.cons[T](this.get,this.next.toStream) // TO BE IMPLEMENTED
}
